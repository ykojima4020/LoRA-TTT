# This file is based on https://github.com/azshue/TPT/blob/main/clip/custom_clip.py. 
# -------------------------------------------------------------------------
...
def tokenize(texts: Union[str, List[str]], context_length: int = 20, truncate: bool = False) -> torch.LongTensor:
    """ 
...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, batch_size=None, n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False):
        ...
        # [NOTE]: clip_model should be Hugging Face instead of original OpenAI.
        if not isinstance(clip_model, transformers.models.clip.modeling_clip.CLIPModel):
            raise TypeError
        ...
        # self.device = clip_model.vision_model.conv1.weight.device
        self.device = clip_model.vision_model.embeddings.patch_embedding.weight.device

        # ctx_dim = clip_model.ln_final.weight.shape[0]
        ctx_dim = clip_model.vision_model.post_layernorm.weight.shape[0]
