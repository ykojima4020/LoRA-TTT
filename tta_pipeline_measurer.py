...
from tta import TTARunner, ParallelTTARunner
from tta.tta_mem_measurer_v2 import ImageEncoderTTAMeasurer, TextPromptTTAMeasurer
# from tta.tta_time_measurer_v2 import ImageEncoderTTAMeasurer, TextPromptTTAMeasurer
...
def build_single_tta_runner(factory, status, config, device='cuda'):
    ...
    if 'tp' in config.keys():
        ...
        handler = TextPromptTTAMeasurer(factory, status, loss, config['tp'])

    elif 'peft' in config.keys():
        # [NOTE]: Choose Loss for PEFT here.
        loss = loss_selector(config['peft'])
        handler = ImageEncoderTTAMeasurer(factory, status, loss, config['peft'], lora=True)
